...
import (
	...
	tokenizer "github.com/cymertek/go-tokenizer"
...
func TestBertPreTokenize_ChineseChars(t *testing.T) {
	...
	for _, r := range n.GetNormalized() {
		if r > 0x4E00 {
			// if normalizer.IsChinese(r) {
			change := []normalizer.ChangeMap{{RuneVal: " ", Changes: 0}, {RuneVal: string(r), Changes: 1}, {RuneVal: " ", Changes: 1}}
			changeMap = append(changeMap, change...)
		} else {
			change := normalizer.ChangeMap{RuneVal: string(r), Changes: 0}
			changeMap = append(changeMap, change)
		}
	}
