...
import (
	...
	"github.com/cymertek/go-tokenizer/pretokenizer"
)
...
func TestProcessorTrimsOffsets(t *testing.T) {
	...
	start := tokenizer.NewEncoding(nil, []int{}, tokens, offsets, nil, nil, nil)
	want := tokenizer.NewEncoding(nil, []int{}, tokens, wantOffsets, nil, nil, nil)
	...
	pairWant := want
	pair := tokenizer.NewEncoding(nil, []int{}, tokens, wantOffsets, nil, nil, nil)
	pairWant.MergeWith(pair, false)

	start1 := tokenizer.NewEncoding(nil, []int{}, tokens, offsets, nil, nil, nil)
	startClone := tokenizer.NewEncoding(nil, []int{}, tokens, offsets, nil, nil, nil)
