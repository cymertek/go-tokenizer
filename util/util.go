...
// StringInSlice checks whether the given string is in the slice.
...
// Contains checks whether the given string is in the slice.
...
// Preserves nil-vs-empty distinction: returns nil only when both inputs are nil.
...
func DeepCopy(src, dist any) (err error) {
	buf := bytes.Buffer{}
...
func GetType(myvar any) string {
	if t := reflect.TypeOf(myvar); t.Kind() == ptrKind {
		return "*" + t.Elem().Name()
	} else {
		return t.Name()
	}
