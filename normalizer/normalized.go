...
import (
	...
	slice "github.com/cymertek/go-tokenizer/util/slice"
...
func (n *NormalizedString) ConvertOffset(inputRange *Range) (retVal *Range) {
	...
	lowerB, upperB := 0, len(alignments)
...
func (n *NormalizedString) TransformRange(inputRange *Range, changeMap []ChangeMap, initialOffset int) (retVal *NormalizedString) {
	...
	if initialOffset > 0 {
		...
		for _, c := range removedChars {
			...
			aligns := append(n.alignmentsOriginal[:removedORange[0]], newAlignments...)
	...
	for _, item := range changeMap {
		...
		var totalBytesToRemove int
		for i := 0; i < nChanges; i++ {
			c, _ := replacedNormalized.Next()
		...
		removingFromOriginal, removingFromNormalized := 0, 0
	...
	if oShift != 0 {
		...
		var endShift [][]int
		endShift = append(endShift, n.alignments[endShiftStart:]...)
...
func (n *NormalizedString) Append(s string) (retVal *NormalizedString) {
	...
	if n.normalized == "" {
		...
		changeMap := []ChangeMap{{string(lastRune), 0}}
		for _, r := range s {
...
func (n *NormalizedString) Split(pattern Pattern, behavior SplitDelimiterBehavior) (retVal []NormalizedString) {
	...
	case MergedWithNextBehavior:
		previousMatch := false
		var acc []OffsetsMatch
		// iterate reversively using slices.Backward for idiomatic Go 1.23+
		...
		// reverse back to original order using slices.Backward
		for _, a := range slices.Backward(acc) {
...
func RangeOf(s string, r []int) (retVal string) {
	...
	// Use min() for cleaner comparison (Go 1.21+)
	end = min(r[len(r)-1], sLen)
