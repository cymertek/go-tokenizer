...
const (
	// WeightName is the default name for model weight files.
	WeightName = "pytorch_model.gt"
	// ConfigName is the default name for configuration JSON files.
	ConfigName = "config.json"
	// TokenizerName is the default name for tokenizer JSON files.
	TokenizerName = "tokenizer.json"

	// NOTE: URL form := `$HFpath/ModelName/resolve/main/WeightName`
	// HFPath is the base URL for HuggingFace model repositories.
	HFpath = "https://huggingface.co"
)

var (
	// DUMMY_INPUT provides a default input tensor used for testing model loading.
...
func CachedPath(modelNameOrPath, fileName string) (resolvedPath string, err error) {
	...
	// http.Get will fail for non-URL paths, so no separate validation needed.
	url := fmt.Sprintf("%s/%s/resolve/main/%s", HFpath, modelNameOrPath, fileName)
	if _, err := http.Get(url); err == nil {
		err := downloadFile(url, cachedFileCandidate)
		if err != nil {
			return "", fmt.Errorf("CachedPath() failed at trying to download file: %w", err)
		}
	...
	// Not resolves
	err = fmt.Errorf("CachedPath() failed: unable to parse %v as URL or local path", url)
	return "", err
}
