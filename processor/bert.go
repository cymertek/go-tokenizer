...
func (bp *BertProcessing) addSpecialToken(encoding *tokenizer.Encoding) *tokenizer.Encoding {
	...
	result := tokenizer.NewEncoding(ids, typeIds, tokens, offsets, specialTokens, attentionMask, []tokenizer.Encoding{}, wordsOpt)

	// Populate SequenceRanges from TypeIds (excluding special token positions).
	if len(result.TypeIds) > 0 && len(result.SequenceRanges) == 0 {
		defaultSequenceID := 0
		result.BuildRangesFromTypeIds(defaultSequenceID, false)
	}
...
// `Overflowing` field. All tokens in the pair are assigned TypeId=1 (sequence B).
func (bp *BertProcessing) pairAddSpecialToken(pairEncoding *tokenizer.Encoding) *tokenizer.Encoding {
	...
	// All tokens in the pair belong to sequence B (type 1), including the [SEP] token.
	var pairTypeIds []int
	for range pairEncoding.Ids {
		pairTypeIds = append(pairTypeIds, 1)
	}
	pairTypeIds = append(pairTypeIds, 1) // SEP also has type 1
