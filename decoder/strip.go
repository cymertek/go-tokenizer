...
// Strip decodes tokens by stripping content within a specified byte range.
type Strip struct {
	*DecoderBase

	Content string // The content to strip
	Start   int    // Start position (inclusive) for stripping
	Stop    int    // Stop position (exclusive) for stripping
}

// NewStrip creates a new Strip decoder that removes Content from positions [Start, Stop).
